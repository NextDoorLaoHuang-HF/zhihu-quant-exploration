...
>
> **小市值策略收益被低估了。** 之前算收益时用的是"未复权价格"——每次股票分红，股价会因除权下调（比如10元派1元，股价变成9元），但代码把这个下调当成了亏损。小市值股票分红多，所以策略收益被严重低估。改用"前复权价格"（把分红自动加回去）重跑后，T5 市值排序年化从 19.82% 修正为 **35.68%**。
>
> **"小盘股持续赢"的结论要打折扣。** 之前的 Fama-French 三因子也有同样的分红计算错误，导致小盘股收益被高估。修正后规模因子（SMB）年化从 21.2% 降到 10.3%，t值从 3.10 降到 1.38——方向仍然为正，但不再达到统计显著（p=0.17）。从"小盘股确实在持续赢"降级为"有方向但不够确凿"。
>
