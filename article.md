...
>
> **之前选的不是"市值最小"而是"价格最低"。** 旧版代码用收盘价排序来代替市值排序——看起来差不多，实际上完全不同。一个10元的股票可能市值100亿（股本大），一个3元的股票可能市值只有5亿（股本小但股价低不等于市值小）。高送转、拆股、退市暴跌都会让价格排序和市值排序严重背离。现在已经改用"未复权价格×流通股本"来算真实市值排序。
>
> **样本从150只随机抽样改成全市场。** 旧版从当前仍在交易的A股里随机抽了150只来跑回测，已退市的股票完全不在里面——这等于只看了"活下来的赢家"，高估了策略收益。现在改用全市场5802只股票（含258只退市股），每个月动态筛选当时还在交易的股票，不再随机抽样。
>
...
顺着"去机构去不了的地方"这个思路，Hermes 做了个实验。拿了全市场5802只A股（含258只已退市股），从2020年跑到2026年7月。每月选市值最小的一批股票，等额买入持有。
