...
>
> **小市值策略的收益算少了。** 之前算收益时用的是"未复权价格"——每次股票分红，股价会因除权下调（比如10元派1元，股价变成9元），但代码把这个下调当成了亏损。小市值股票分红多，所以策略收益被严重低估。改用"前复权价格"（把分红自动加回去）重跑后，T5 市值排序年化从 19.82% 修正为 **35.68%**。
>
> **但"小盘股持续赢"这个统计结论反而变弱了。** Fama-French 三因子模型不是看策略绝对收益，而是看小盘股相对大盘股是否有持续超额。同样的分红计算错误在小盘股上影响更大，导致旧版 SMB（小盘减大盘）被虚高。修正后 SMB 年化从 21.2% 降到 10.3%，t值从 3.10 降到 1.38——方向仍然为正，但不再达到统计显著（p=0.17）。从"小盘股确实在持续赢"降级为"有方向但不够确凿"。
>
> **网格交易结论从"全输"修正为"29%胜率"。** 之前的网格交易回测有记账错误（卖出时多乘了一次价格，虚增了现金），导致结论自相矛盾。修复账本后重跑：72组中21组跑赢买持（胜率29%），跑赢的是震荡型品种，跑输的是单边上涨品种。
>
> **填权超额从-2.3%修正为+2.79%。** 之前的填权研究用同一股票的涨跌做基准（等于自己比自己），改为用全市场指数做基准后，60天填权超额从-2.3%变为+2.79%——方向反过来了，但幅度很小，不足以作为独立策略。
>
> **可转债策略样本扩充。** 可转债池从4只扩充到1006只（含682只已退出券），策略重命名为"可转债等权/CB-HRP混合策略"（ETF仅用于日期对齐，不参与收益计算）。OOS年化6.62%，夏普0.59。
>
...
在除权日买入，持有60天后：平均赚3.19%，剔除市场涨跌后超额是+2.79%。2943次除权事件中，80%在60天后真的填权了（涨回了除权前的价格）。
