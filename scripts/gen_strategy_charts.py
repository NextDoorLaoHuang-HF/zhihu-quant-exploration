...
fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(nav(t5[common]).index, nav(t5[common]).values, color='#dc2626', linewidth=2.5, label='纯T5微盘 (年化23.9%)')
ax.plot(nav(mix50).index, nav(mix50).values, color='#2563eb', linewidth=2, label='T5-50% + 国债-50% (年化13.3%)')
ax.plot(nav(mix80).index, nav(mix80).values, color='#7c3aed', linewidth=1.5, label='T5-80% + 国债-20% (年化19.7%)')
...
fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(nav(t5).index, nav(t5).values, color='#dc2626', linewidth=2, label='纯T5微盘 (年化23.9%)')
ax.plot(nav(rot).index, nav(rot).values, color='#2563eb', linewidth=2, label='策略轮动 (波动>25%→国债) (年化17.7%)')
