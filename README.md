...
| [D. 公积金取 vs 留](#项目d公积金有必要取出来吗) | [Q564845718](https://www.zhihu.com/question/564845718) | ✅ 数字逐条溯源+年报反推互验 |
...
对应知乎问题[「公积金有必要取出来吗？」](https://www.zhihu.com/question/564845718)（58答 / 212.8万浏览）的回答配套计算。不是回测，是**政策数字核查 + 取/留机会成本算账**。

**核查动机**：该问题下利率口径三个版本打架（0.4% / 1.5% / 2.6%），顶置1010赞为2022年答案且自标"已变更"，"2026公积金改革提取限制取消"的说法流传但无出处。本项目把每个数字落到官方来源。

**核实结论（2026-08-01，逐条来源见 `data/gjj_verified_sources.json`）**：

| 数字 | 值 | 来源 |
|---|---|---|
| 公积金存款利率 | **1.5%**（一年期定存基准利率） | 条例第21条+央行2016年机制；**年报反推互验**：2024年支付缴存人利息1597.29亿÷平均缴存余额≈**1.52%** |
| 公积金贷款利率·首套 | 2.1%（5年以下）/ **2.6%**（5年以上），2025-05-08起 | 北京中心官网历年利率表（2002-2025） |
| 一年定存挂牌 | 0.95% | 中行2025-05-20利率表（最新一期） |
| 余额宝7日年化 | 1.21% | 天天基金000198 |
| LPR | 3.0%（1Y）/ 3.5%（5Y+），2026-07-20 | 中国货币网 |
| "2026改革" | 国家定调"深化制度改革、扩大使用范围"（《扩大消费"十五五"规划》2026-07-13批复）；**无"全国统一取消提取限制"文件**，放宽由各地陆续出台 | gov.cn |

**算账结果（`results/gjj_withdrawal/results.json`）**：

- 纯利息：10万×10年，留公积金（1.5%）比取出放余额宝（1.21%）多约 3,273 元——利息差是小头
- 贷款资格：贷100万×30年等额本息，公积金（2.6%）比商贷（LPR 3.5%）少还 **17.5万**利息（利差0.45~1.5pct情景下 8.6万~29.8万）——这才是大头
- "取出来放余额宝更划算"的建议（基于2022年余额宝2%+）在现行利率下已反转：1.21% < 1.5%

**复现**：

```bash
python3 scripts/gjj_withdrawal_calc.py   # 计算+2图 → results/gjj_withdrawal/ + charts/gjj_*.png
```

图表：`charts/gjj_rate_landscape.png`（利率全景）、`charts/gjj_loan_gap.png`（贷款利息差）。
