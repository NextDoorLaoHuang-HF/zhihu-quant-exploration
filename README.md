...
**读者提问变体2：股息率仓位管理（>5%越高越买，<4%越低越卖）**（`bank_dividend_yield_band.py`）：

10万存量、目标仓位=clamp((股息率-4%)/2%, 0, 1)、每月调仓、卖出计佣金+印花税0.1%。**结论：不会更高**。工行2010-08起：线性调仓 8.2% vs 买入持有 9.25%（19年买卖70+次、成交额180万，磨损+牛市半仓踏空）；缓冲带版（4-5%不动）9.54%≈持有；四行组合缓冲带10.20% vs 持有10.05%（+0.15pct≈打平，建行单品种反而跑输1.25pct）。唯一大幅跑赢是2007大顶起点（8.26% vs 4.42%），但几乎全部来自起点空仓避开腰斩的后视镜。⚠️ 初版缓冲带曾显示11.16%，追查调仓明细发现1.6pct来自**TTM接缝伪影**（跨年除权日错位→窗口无派息→股息率瞬间为0→"神级逃顶"2015/2021），修正口径后优势消失；阈值±0.5pct结果在7.5%~11.2%乱跳——择时优势藏在数据伪影和参数运气里，不是真信号。
